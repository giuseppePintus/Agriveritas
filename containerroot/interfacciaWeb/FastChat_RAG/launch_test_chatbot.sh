...
# Start worker
CUDA_VISIBLE_DEVICES=0 python3 -m fastchat.serve.model_worker --model-path $MODEL_NAME 1>/dev/null 2>&1 &

echo "----------------------------------------------------------------"
echo "Testing everything is fine"
CUDA_VISIBLE_DEVICES=0  python3 -m fastchat.serve.test_message --model-name $MODEL_NAME
echo "----------------------------------------------------------------"
...
CUDA_VISIBLE_DEVICES=0 python3 -m fastchat.serve.model_worker --model-path meta-llama/Meta-Llama-3-8B-Instruct --controller http://127.0.0.1:21001 --host 127.0.0.1 --port 31001 --worker http://127.0.0.1:31001 >logs/worker_gemma_logs 2>&1 &